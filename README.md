...
- **Principal Component Analysis** [[Code]](src/dim/pca/pca.py): Principal component analysis (PCA) is an unsupervised linear technique for dimensionality reduction on a data set. The principal components of a data set are a set of orthonormal vectors that maximize the variance of the data after projecting onto them. PCA is the process of computing these principal components and then projecting each data point onto the top k principal components to obtain lower-dimensional data while preserving as much of the data's variation.
