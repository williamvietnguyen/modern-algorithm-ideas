...
- **Principal Component Analysis** [[Code]](src/dimreduction/pca/pca.py): Principal component analysis (PCA) is an unsupervised linear technique for dimensionality reduction on a data set. The principal components of a data set are p vectors, where the i<sup>th</sup> vector is the vector that best fits the data while being orthogonal to the first i - 1 vectors. These vectors form an orthonormal basis where the dimensions are linearly uncorrelated. PCA is the computation of these components and using them to perform a change of basis on the data, often using only the first few components. These principal components are ordered by their ability to maximize the variance of the projected data.
